...
class CacheManager:
    def __init__(self, root: str, dbname: str, cache_format: str, compress: bool, partitioning: str, compression_level: int | None = None, parquet_codec: str | None = None):
        ...
        self.dbname = dbname
        self.cache_format = cache_format  # "csv" | "parquet"
    ...
    def table_root(self, table: str) -> str:
        p = os.path.join(self.root, self.dbname, table)
        ensure_dir(p)
        return p

    def derived_root(self, cache_key: str) -> str:
        p = os.path.join(self.root, self.dbname, "_derived", cache_key)
        ensure_dir(p)
        return p
    ...
    def partition_path(self, base_root: str, ts: pd.Timestamp) -> str:
        ...
        if self.partitioning == "none":
            ...
            sub = os.path.join(base_root, f"{ts.year:04d}")
            ensure_dir(sub)
            return os.path.join(sub, f"{day_key(ts)}{ext}")
        else:  # monthly
            sub = os.path.join(base_root, f"{ts.year:04d}")
            ensure_dir(sub)
            return os.path.join(sub, f"{month_key(ts)}{ext}")
    ...
    def read_df_if_exists(self, path: str, usecols: list[str] | None = None) -> pd.DataFrame:
        ...
        if path.endswith(".gz"):
            return pd.read_csv(path, usecols=usecols, compression="gzip")
    ...
    def write_df_atomic(self, path: str, df: pd.DataFrame) -> None:
        ...
        if self.cache_format == "parquet":
            ...
            compression = self.parquet_codec  # "snappy" (default if compress) | "zstd" | None
            df.to_parquet(tmp, index=False, engine="pyarrow", compression=compression)
        else:
            if path.endswith(".gz"):
                comp = {"method": "gzip"}
                if self.compression_level is not None:
                    comp["compresslevel"] = int(self.compression_level)
