...
class Adapter:
    ...
    def build_incremental_query(
        ...
        start: pd.Timestamp, end: pd.Timestamp, datetime_column: str,
        where: str | None, order_by: str | None
    ) -> tuple[str, dict]:
        cols = "*" if not columns else ", ".join([self.quote_ident(c) for c in columns])
        sql = [
            "SELECT", cols, "FROM", self.quote_ident(table), "WHERE",
            f"{self.quote_ident(datetime_column)} >= :start AND {self.quote_ident(datetime_column)} < :end"
        ]
        if where:
            sql.extend(["AND", where])
        if order_by:
    ...
    def build_segmented_query(
        ...
        for i, (s, e) in enumerate(segments):
            params[f"s{i}"] = s.to_pydatetime()
            params[f"e{i}"] = e.to_pydatetime()
