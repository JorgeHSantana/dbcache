...
class PostgresAdapter(Adapter):
    ...
    def _prepare_driver(self):
        try:
            ...
            self._connect_fn = psycopg.connect
            return
        except Exception:
            pass
        try:
    ...
    def execute_select(self, sql: str, params: dict | None = None) -> pd.DataFrame:
        sql, params = (sql, params or {})
        sql, params = expand_in_clause(sql, params)
        sql = to_psycopg_named(sql)
        conn = self.connect()
        try:
            if self._driver == "psycopg":
                with conn.cursor() as cur:
    ...
    def peek_columns_sql(self, sql: str, params: dict | None = None) -> list[str]:
        ...
        wrapped, p = expand_in_clause(wrapped, params or {})
        wrapped = to_psycopg_named(wrapped)
        conn = self.connect()
        try:
            if self._driver == "psycopg":
                with conn.cursor() as cur:
                    cur.execute(wrapped, p)
                    return [d[0] for d in cur.description]
            else:
                with conn.cursor() as cur:
                    cur.execute(wrapped, p)
    ...
    def build_segmented_query(
        self, table: str, columns: list[str] | None, datetime_column: str,
        ...
        sql = [
            ...
            ")",
            f"SELECT {cols}",
            f"FROM {self.quote_ident(table)} t",
        ...
        params = {
            "starts": [s.to_pydatetime() for s, _ in segments],
            "ends":   [e.to_pydatetime() for _, e in segments],
        }
        return "\n".join(sql), params
