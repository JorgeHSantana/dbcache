...
import typing as T, os, pandas as pd
...
def _iter_days(start_ts: pd.Timestamp, end_ts: pd.Timestamp):
    ...
    while cur < end_ts: 
        yield cur; cur += pd.Timedelta(days=1)
...
def _iter_months(start_ts: pd.Timestamp, end_ts: pd.Timestamp):
    ...
    while cur < end_ts:
        yield cur
        cur = pd.Timestamp(year=cur.year + (1 if cur.month==12 else 0), month=(1 if cur.month==12 else cur.month+1), day=1, tz="UTC")

def _group_consecutive(ts_list: list[pd.Timestamp], step: str) -> list[tuple[pd.Timestamp, pd.Timestamp]]:
    segs=[]; 
    if not ts_list: return segs
    ts_list=sorted(ts_list); start=ts_list[0]; prev=start
    for ts in ts_list[1:]:
        expected = prev + (pd.Timedelta(days=1) if step=="daily" else (pd.Timestamp(year=prev.year + (1 if prev.month==12 else 0), month=(1 if prev.month==12 else prev.month+1), day=1, tz="UTC") - prev))
        if ts != expected: segs.append((start, expected)); start=ts
        prev=ts
    if step=="daily": segs.append((start, prev + pd.Timedelta(days=1)))
    else:
        end = pd.Timestamp(year=prev.year + (1 if prev.month==12 else 0), month=(1 if prev.month==12 else prev.month+1), day=1, tz="UTC"); segs.append((start, end))
    return segs

class DataBase:
    def __init__(self, url: str, port: int, user: str, password: str, name: str, *, path: str = "database/", db_type: str = "postgres", cache_format: str = "csv", compress: bool = False, datetime_tz: str | None = None, lookback: str = "15min", partitioning: str = "monthly", index_columns: list[str] | None = None, file_lock_timeout: float = 30.0, logger: T.Any | None = None, compression_level: int | None = None, parquet_codec: str | None = None, on_new_columns: str = "error"):
        # compat param
        if on_new_columns != "error":
            raise ValueError("Only 'error' is supported for on_new_columns in v0.2.5 (schema is fixed).")
        ...
        self.compression_level=compression_level; self.parquet_codec=parquet_codec; self.on_new_columns=on_new_columns
        if self.db_type=="postgres": self.adapter=PostgresAdapter(self.host, self.port, self.user, self.password, self.dbname, self.datetime_tz)
        else: raise NotImplementedError(f"db_type '{self.db_type}' not supported.")
        if self.cache_format not in {"csv","parquet"}: raise NotImplementedError("cache_format must be 'csv' or 'parquet'.")
    ...
    def _ensure_schema_or_raise_select(self, base_root: str, sql: str, params: dict) -> list[str]:
        cached_cols = self._get_cached_schema(base_root); query_cols = self.adapter.peek_columns_sql(sql, params or {})
        if cached_cols is not None and set(cached_cols) != set(query_cols):
            raise SchemaMismatchError(f"Cached schema {cached_cols} != query schema {query_cols}")
        if cached_cols is None: self._set_cached_schema_if_absent(base_root, query_cols)
        return query_cols

    def _ensure_schema_or_raise_table(self, base_root: str, table: str, columns: list[str] | None) -> list[str]:
        cached_cols = self._get_cached_schema(base_root); intended_cols = columns if (columns and len(columns)>0) else None
        if intended_cols is None:
            intended_cols = self.adapter.peek_table_columns(table, None) if cached_cols is None else cached_cols
        if cached_cols is not None and set(cached_cols) != set(intended_cols):
            raise SchemaMismatchError(f"Cached schema {cached_cols} != requested columns {intended_cols}")
        if cached_cols is None: self._set_cached_schema_if_absent(base_root, list(intended_cols))
        return list(intended_cols)

    def get_table(self, table: str, columns: list[str] | None, start: pd.Timestamp | str, end: pd.Timestamp | str, datetime_column: str, where: str | None = None, order_by: str | None = None) -> pd.DataFrame:
        ...
        base_root = self.cache.table_root(table); _ = self._ensure_schema_or_raise_table(base_root, table, columns)
        ...
        if self.partitioning=="daily":
            ...
            for day in _iter_days(start_ts, end_ts):
                ...
                if not os.path.exists(part_path): missing_days.append(day)
            ...
            for month in _iter_months(start_ts, end_ts):
                ...
                if not os.path.exists(part_path): missing_months.append(month)
            missing_segments = _group_consecutive(missing_months, "monthly")
        if not missing_segments:
            fetch_from = max(start_ts, pd.to_datetime(cached[datetime_column], utc=True, errors="coerce").max() - self.lookback) if (not cached.empty and datetime_column in cached.columns) else start_ts
            sql, params = self.adapter.build_incremental_query(table=table, columns=columns, start=fetch_from, end=end_ts, datetime_column=datetime_column, where=where, order_by=order_by)
            self._ensure_only_select(sql); delta = self.adapter.execute_select(sql, params)
            merged = self._merge_and_dedupe(cached, delta, key_cols=self._dedup_keys(datetime_column)); self._write_partitions_window(base_root, merged, datetime_column)
            return self._filter_window(merged, datetime_column, start_ts, end_ts, order_by).reset_index(drop=True)
        lb=self.lookback; segs=[]
        for s,e in missing_segments:
            seg_start=max(start_ts, s - lb); seg_end=min(end_ts, e + lb)
            if seg_start < seg_end: segs.append((seg_start, seg_end))
        sql, params = self.adapter.build_segmented_query(table=table, columns=columns, datetime_column=datetime_column, segments=segs, where=where, order_by=order_by)
        self._ensure_only_select(sql); delta=self.adapter.execute_select(sql, params)
        merged=self._merge_and_dedupe(cached, delta, key_cols=self._dedup_keys(datetime_column)); self._write_partitions_window(base_root, merged, datetime_column)
        return self._filter_window(merged, datetime_column, start_ts, end_ts, order_by).reset_index(drop=True)

    def select(self, sql: str, params: dict | list | tuple | None = None, *, cache_hint: CacheHint | None = None, cache_mode: str = "auto", order_by: str | None = None) -> pd.DataFrame:
        ...
        ch=cache_hint; base_root=self.cache.derived_root(ch.cache_key); p=dict(params or {})
        ...
        if start_name not in p or end_name not in p:
            if cache_mode=="force": raise ValueError(f"Cache in 'force' requires params '{start_name}' and '{end_name}'.")
        ...
        cached = self._read_partitions_window(base_root, start_ts, end_ts, usecols=None); lb = parse_lookback(ch.lookback) if ch.lookback else self.lookback
        ...
        if ch.partitioning=="daily":
            ...
            for month in _iter_months(start_ts, end_ts):
                ...
                if not os.path.exists(part_path): missing_months.append(month)
            missing_segments = _group_consecutive(missing_months, "monthly")
        else: missing_segments=[]
        if not missing_segments:
            fetch_from = max(start_ts, pd.to_datetime(cached[ch.datetime_column], utc=True, errors="coerce").max() - lb) if (not cached.empty and ch.datetime_column in cached.columns) else start_ts
            delta = self.adapter.execute_select(sql, p)
            if ch.datetime_column not in delta.columns: raise KeyError(f"Temporal column '{ch.datetime_column}' (CacheHint) not in result.")
            merged = self._merge_and_dedupe(cached, delta, key_cols=ch.index_columns or []); self._write_partitions_window(base_root, merged, ch.datetime_column)
    ...
    def _dedup_keys(self, datetime_column: str) -> list[str]:
        ...
        if not keys: keys=[datetime_column]; return keys
        return keys
    ...
    def _write_partitions_window(self, base_root: str, df: pd.DataFrame, datetime_column: str) -> None:
        if df.empty: return
        if datetime_column not in df.columns: raise KeyError(f"Temporal column '{datetime_column}' not in DataFrame.")
        ...
        if self.partitioning=="none":
            ...
            with FileLock(path, timeout=self.file_lock_timeout): self.cache.write_df_atomic(path, dft.sort_values(by=datetime_column))
            return
        if self.partitioning=="daily":
            ...
            for day, part in dft.groupby("_dkey"):
                ...
                with FileLock(path, timeout=self.file_lock_timeout):
                    old=self.cache.read_df_if_exists(path); merged=self._merge_and_dedupe(old, part.drop(columns=["_dkey"]), key_cols=self.index_columns)
                    self.cache.write_df_atomic(path, merged.sort_values(by=datetime_column))
        else:
            s=pd.to_datetime(dft[datetime_column], utc=True, errors="coerce"); dft["_mkey"]=s.dt.tz_localize(None).dt.to_period("M")
            for mkey, part in dft.groupby("_mkey"):
                ...
                with FileLock(path, timeout=self.file_lock_timeout):
                    old=self.cache.read_df_if_exists(path); merged=self._merge_and_dedupe(old, part.drop(columns=["_mkey"]), key_cols=self.index_columns)
                    self.cache.write_df_atomic(path, merged.sort_values(by=datetime_column))
    def _merge_and_dedupe(self, cached: pd.DataFrame, delta: pd.DataFrame, key_cols: list[str]) -> pd.DataFrame:
        if cached.empty: base=delta.copy()
        elif delta.empty: base=cached.copy()
        else: base=pd.concat([cached, delta], ignore_index=True)
        if not key_cols: return base
    ...
    def _filter_window(self, df: pd.DataFrame, datetime_column: str, start_ts: pd.Timestamp, end_ts: pd.Timestamp, order_by: str | None) -> pd.DataFrame:
        if df.empty or datetime_column not in df.columns: return df
        dft=df.copy(); dft[datetime_column]=pd.to_datetime(dft[datetime_column], utc=True)
        mask=(dft[datetime_column] >= start_ts.tz_convert("UTC")) & (dft[datetime_column] < end_ts.tz_convert("UTC"))
        dft=dft.loc[mask]
        if order_by:
            cols=[c.strip().split()[0] for c in order_by.split(",")]; ascending=[("DESC" not in c.upper()) for c in order_by.split(",")]
